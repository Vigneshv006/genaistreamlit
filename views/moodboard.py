...
# image_number_input = st.text_input("Enter Image Number:", value="1")

# # Validate the input to ensure it's a positive integer
# try:
#     image_number = int(image_number_input)
#     if image_number < 1:
...
#     image_review = st.slider(f"Rate Image {image_number}:", 1, 10, value=default_rating, format="%d")
...
#                 # Convert the current prompt feedback text to a default slider rating
#         default_rating = 10 if prompts_df.iloc[selected_prompt_index]['prompt_feedback'] == "GOOD" else 1
        
#         # Add a slider to rate the prompt from 1 to 10
...
#         # Map slider rating back to "GOOD" or "BAD"
...
# At the top of your script, add this state initialization
if "image_number" not in st.session_state:
    st.session_state.image_number = 1
